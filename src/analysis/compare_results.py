...
This script:
- Loads results from output/mle_results/, output/parameter_estimates/,
    output/bayesian_results/
...
if COMPARISON_OUTPUT_DIR.exists():
    ...
    print(
        f"Cleaned existing comparison output directory: "
        f"{COMPARISON_OUTPUT_DIR}"
    )
...
if mle_file.exists():
    ...
    print(f"[OK] Loaded MLE results: {len(df_mle)} trees")
    # Rename columns to have mle_ prefix
    mle_cols = {col: f'mle_{col}' if col not in ['tree_idx', 'tip_size',
        'true_lambda', 'true_mu'] 
                else col for col in df_mle.columns}
    df_mle = df_mle.rename(columns=mle_cols)
else:
    print(f"[FAIL] MLE results not found: {mle_file}")
...
if phylodeep_file.exists():
    ...
    print(f"[OK] Loaded PhyloDeep results: {len(df_phylodeep)} trees")
    ...
    print(f"[FAIL] PhyloDeep results not found: {phylodeep_file}")
    print("  Run phylodeep_analysis.py first")
    df_phylodeep = None

# Load Bayesian results
    bayesian_file = (
        PROJECT_ROOT / "output" / "bayesian_results" /
        "all_bayesian_results.csv"
    )
if bayesian_file.exists():
    df_bayesian = pd.read_csv(str(bayesian_file))
    print(f"[OK] Loaded Bayesian results: {len(df_bayesian)} trees")
    ...
    print(f"[WARN] Bayesian results not found: {bayesian_file}")
...
if df_mle is None and df_phylodeep is None:
    print("\n[FAIL] No results found! Run at least one analysis script first.")
    sys.exit(1)
...
if df_phylodeep is not None and df_mle is not None:
    ...
    print(f"[OK] Merged PhyloDeep results")
...
if df_bayesian is not None:
    ...
    print(f"[OK] Merged Bayesian results")
...
for tip_size in tip_sizes:
    ...
    if ('mle_lambda' in df_size.columns and
        'phylodeep_lambda' in df_size.columns):
        ...
        if len(mle_lambda_valid) > 0 and len(phylodeep_lambda_valid) > 0:
            ...
            comparison_stats.append({
                ...
                'n_estimates': min(len(mle_lambda_valid),
                    len(phylodeep_lambda_valid))
            })
        ...
        if len(mle_mu_valid) > 0 and len(phylodeep_mu_valid) > 0:
            ...
            # Efficiency
            efficiency_mu = (
                mle_mu_var / phylodeep_mu_var
                if phylodeep_mu_var > 0 else np.nan
            )
    ...
    for tip_size in tip_sizes[:5]:  # Show first 5 tip sizes
        ...
        for param in ['lambda', 'mu']:
            ...
            if len(eff_stats) > 0:
                ...
                print(
                    f"    Statistical Efficiency "
                    f"(Var(MLE)/Var(PhyloDeep)): {eff:.4f}"
                )
                if eff > 1:
                    print(
                        f"      -> PhyloDeep is {eff:.2f}x more efficient "
                        "(lower variance)"
                    )
                elif eff < 1:
                    print(
                        f"      -> MLE is {1/eff:.2f}x more efficient "
                        "(lower variance)"
                    )
...
try:
    ...
    fig.suptitle('MLE vs PhyloDeep Parameter Estimates Comparison',
        fontsize=16)
    
    # Lambda comparison
    if ('mle_lambda' in df_comparison.columns and
        'phylodeep_lambda' in df_comparison.columns):
        ax1 = axes[0, 0]
        ...
        if len(mle_lambda) > 0 and len(phylodeep_lambda) > 0:
            ...
            if len(merged_lambda) > 0:
                ...
                ax1.plot([min_val, max_val], [min_val, max_val], 'r--',
                    label='y=x')
                ax1.set_xlabel('MLE lambda')
                ax1.set_ylabel('PhyloDeep lambda')
                ax1.set_title('Lambda (Birth Rate) Estimates')
                ax1.legend()
                ax1.grid(True, alpha=0.3)
    
    # Mu comparison
    if ('mle_mu' in df_comparison.columns and
        'phylodeep_mu' in df_comparison.columns):
        ax2 = axes[0, 1]
        merged_mu = pd.merge(
        ...
        if len(merged_mu) > 0:
            ax2.scatter(merged_mu['mle_mu'], merged_mu['phylodeep_mu'],
                alpha=0.5, s=20)
            min_val = min(merged_mu['mle_mu'].min(),
                merged_mu['phylodeep_mu'].min())
            max_val = max(merged_mu['mle_mu'].max(),
                merged_mu['phylodeep_mu'].max())
            ax2.plot([min_val, max_val], [min_val, max_val], 'r--',
                label='y=x')
            ax2.set_xlabel('MLE mu')
            ax2.set_ylabel('PhyloDeep mu')
    ...
    if len(comparison_stats) > 0:
        ...
        efficiency_data = df_stats_plot[
            df_stats_plot['method'] == 'Efficiency'
        ].copy()
        if len(efficiency_data) > 0:
            lambda_eff = efficiency_data[
                efficiency_data['parameter'] == 'lambda'
            ]
            mu_eff = efficiency_data[efficiency_data['parameter'] == 'mu']
            
            if len(lambda_eff) > 0:
                ax3.plot(lambda_eff['tip_size'],
                    lambda_eff['efficiency_ratio'], 
                        'o-', label='lambda efficiency', markersize=8)
            if len(mu_eff) > 0:
                ax3.plot(mu_eff['tip_size'], mu_eff['efficiency_ratio'], 
                        's-', label='mu efficiency', markersize=8)
    ...
    # Bias comparison
    ax4 = axes[1, 1]
    if ('true_lambda' in df_comparison.columns and
        df_comparison['true_lambda'].notna().any()):
        df_with_bias = df_comparison[
            df_comparison['true_lambda'].notna()
        ].copy()
        if ('mle_lambda' in df_with_bias.columns and
            'phylodeep_lambda' in df_with_bias.columns):
            ...
            if len(mle_bias) > 0 and len(phylodeep_bias) > 0:
                ax4.hist(mle_bias, bins=30, alpha=0.5, label='MLE bias',
                    density=True)
                ax4.hist(
                    phylodeep_bias, bins=30, alpha=0.5,
                    label='PhyloDeep bias', density=True
                )
...
print(
    "  - comparison_statistics.csv: Summary statistics "
